(function (spriteUrl) {
  var ROOT_ID = 'dino-chicken-root';

  if (window.__dinoChicken) {
    window.__dinoChicken.destroy();
    return;
  }
  ...
  style.textContent =
    ...
    '}' +
    '#' + ROOT_ID + ' .dino-chicken-hint{' +
      'position:fixed;top:16px;left:16px;padding:12px 16px;' +
      'background:rgba(0,0,0,.55);color:#fff;font:14px/1.5 Helvetica Neue,sans-serif;' +
      'border-radius:6px;pointer-events:auto;' +
    '}' +
    '#' + ROOT_ID + ' .dino-chicken-stage{' +
      'position:absolute;inset:0;' +
    '}' +
    '#' + ROOT_ID + ' .dino-chicken-sprite{' +
  ...
  root.id = ROOT_ID;
  root.innerHTML =
    '<div class="dino-chicken-hint">arrow keys move &middot; space peck &middot; click bookmark again to remove</div>' +
    '<div class="dino-chicken-stage"><div class="dino-chicken-sprite"></div></div>';

  var gameEl = root.querySelector('.dino-chicken-stage');
  var chickenEl = root.querySelector('.dino-chicken-sprite');
  ...
  window.__dinoChicken = { destroy: destroy };
})(__SPRITE_URL__);
